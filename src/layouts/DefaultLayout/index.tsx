...
const DefaultLayout = () => {
  ...
  const items = [
    ...
    },
    {
      ...
      key: '/whatsapp',
      submenus: [
        { label: 'Grupos', key: '/whatsapp/grupos' },
        { label: 'Categorias', key: '/whatsapp/categorias' },
      ],
    },
    {
      ...
      key: '/discord',
      submenus: [
        { label: 'Servidores', key: '/discord/servidores' },
        { label: 'Categorias', key: '/discord/categorias' },
      ],
    },
    {
      ...
      key: '/telegram',
      submenus: [
        { label: 'Grupos', key: '/telegram/grupos' },
        { label: 'Categorias', key: '/telegram/categorias' },
      ],
    },
  ];

  const generateMenuItems = (items : any) =>
    items.map(({ submenus, label, key, icon } : any) => ({
      label,
      key,
      icon,
      children: submenus
        ? submenus.map(({ label: submenuLabel, key: submenuKey } : any) => ({
            label: (
              <div
                className={
                  getIsActivePathname(submenuKey)
                    ? styles.menuItemActive
                    : styles.menuItem
                }
              >
                <Link to={submenuKey}>{submenuLabel}</Link>
              </div>
            ),
            key: submenuKey,
          }))
  ...
  return (
    <Layout className={styles.layout}>
      ...
      >
        <Title className={styles.logo} level={2}>
          {collapsed ? 'G' : 'Groopify'}
        </Title>
        <Menu
      ...
      <Layout
        ...
        style={{ marginLeft: collapsed ? 100 : 250 }}
      >
