...
const TCategoryListPage: React.FC = () => {
  ...
  const categoriesList = async (page: number, pageSize: number) => {
    try {
      const res = await api.get(`/categories/Telegram/trending/paginated?page=${page}&perPage=${pageSize}`);
  ...
  const searchCategories = async (page : number, pageSize: number, search: string) => {
    try {
      const res = await api.get(`/categories/Telegram/${search}/search/paginated?page=${page}&perPage=${pageSize}`);
      setCategories(res.data.rows.data);
      setTotal(res.data.rows.total);
    } catch (error) {
      console.log(error);
